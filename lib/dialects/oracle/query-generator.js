  ...
  var QueryGenerator = {
      ...
      for (key in valueHash) {
        if (valueHash.hasOwnProperty(key)) {
          ...
          if (modelAttributeMap && modelAttributeMap[key] && !value) {
            ...
            if (modelAttributeMap[key].returning) {
              if (modelAttributeMap[key].type instanceof DataTypes.NUMBER) {
                bindType = this.sequelize.connectionManager.lib.NUMBER;
              }else if (modelAttributeMap[key].type instanceof DataTypes.UUID) {
                bindType = this.sequelize.connectionManager.lib.UUID;
    ...
    bulkInsertQuery: function (tableName, attrValueHashes, options) {
      ...
      var attributes = Utils._.template(attributesTpl)({
        table: this.quoteIdentifier(tableName.tableName || tableName),
